...
def subdomain_enum(target_url, wordlist_path, timeout=5):
    try:
    ...
    print(f"[*] Toplam {len(words)} subdomain deneniyor...\n")
    ...
    def check_subdomain(subdomain):
        full_url = f"http://{subdomain}.{target_url}"
        try:
    ...
    print(f"[*] Subdomain taraması başlatıldı...\n")
    start_threads()


target = input("Hedef site (örn: example.com): ").strip()
...
subdomain_enum(target, wordlist, timeout)
